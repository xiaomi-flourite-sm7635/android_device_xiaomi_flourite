...
class NonUiSensorCallback : public IEventQueueCallback {
  public:
    NonUiSensorCallback() {
        ...
        if (ioctl(touch_fd_.get(), xiaomi::touch::kSelectTouchIdIoctl,
                  xiaomi::touch::kPrimaryTouchId) == -1) {
            PLOG(ERROR) << "failed to select the primary touchscreen";
            touch_fd_.reset();
        }
    }
    ...
    Return<void> onEvent(const Event& e) {
        ...
        xiaomi::touch::CommonData request = {
            .touchId = xiaomi::touch::kPrimaryTouchId,
            .command = xiaomi::touch::kSetCurrentValue,
            .mode = xiaomi::touch::kNonUiMode,
            .dataLength = 1,
            .data = {static_cast<int32_t>(e.u.scalar)},
        };
        if (ioctl(touch_fd_.get(), xiaomi::touch::kCommonDataIoctl, &request) == -1) {
            PLOG(ERROR) << "failed to update non-UI mode";
        }
