...
ALLOCATOR_V1_NEEDED = "android.hardware.graphics.allocator-V1-ndk.so"
...
def mark_allocator_v1_blobs(
    stock_root: Path, entries: dict[str, str]
) -> None:
    """Keep stock V1 DT_NEEDED entries without triggering Soong conflicts."""
    for path in sorted(entries):
        if ALLOCATOR_V1_NEEDED not in needed_libraries(stock_root / path):
            continue
        line = entries[path]
        if "DISABLE_DEPS" not in line and "DISABLE_CHECKELF" not in line:
...
def main() -> None:
    ...
    mark_allocator_v1_blobs(stock_root, entries)
