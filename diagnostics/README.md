...
The V10 logger must start before the normal `first_stage_console` call, because
...
platform patch. V10 keeps the console supervisor's `SIGCHLD` handling isolated
...
V10 keeps capturing after `adbd` or `system_server` first appear and syncs the
record every second. This preserves the final crash loop when PID 1 takes its
five-second fatal-reboot path. It writes those detailed snapshots directly to
`oops` so `/dev/kmsg` cannot rate-limit them. Normal builds leave the logger and
both bootconfig parameters out.
